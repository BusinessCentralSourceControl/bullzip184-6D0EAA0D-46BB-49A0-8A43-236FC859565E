Table 6188711 "ForNAV Inventory to G/L Buffer"
{
    fields
    {
    ...
    procedure CreateValues(var Args: Record "ForNAV Inv. to G/L Arguments";
    Received: Boolean;
    Shipped: Boolean)var WrongCallErr: label 'Wrong Function Call', Comment='DO NOT TRANSLATE';
    CheckTemporary: Codeunit "ForNAV Check Temporary";
    begin
        if(Received = Shipped) or (CheckTemporary.IsTemporary(Rec, false))then Error(WrongCallErr);
        if "Valued Quantity" = 0 then "Valued Quantity":="Invoiced Quantity"
        else if "Valued Quantity" > 0 then begin
            ...
            else if "Valued Quantity" < 0 then if Args."Amounts in Add. Currency" then begin
