...
{
    Caption = 'Check';
    WordLayout = './Layouts/ForNAV US Check.docx';
    DefaultLayout = Word;
    ...
    dataset
    {
        dataitem(Args;"ForNAV Check Arguments")
        {
            ...
            dataitem(VoidGenJnlLine;"Gen. Journal Line")
            {
                ...
                var TestVoidCheck: Codeunit "ForNAV Test Void Check";
                begin
                    VoidGenJnlLine.SetFilter("Bal. Account No.", '%1|%2', '', Args."Bank Account No.");
                    if not TestVoidCheck.TestVoidCheck(VoidGenJnlLine, Args, CurrReport.Preview)then CurrReport.Break;
                    ReportForNav.OnPreDataItem('VoidGenJnlLine', VoidGenJnlLine);
                end;
                trigger OnAfterGetRecord();
                var CheckManagement: Codeunit CheckManagement;
                begin
            ...
            dataitem(GenJnlLnBuffer;"Gen. Journal Line")
            {
                ...
                trigger OnAfterGetRecord();
                begin
                    if not Args.CreateModelFromGenJnlLn(GenJnlLnBuffer, Model)then CurrReport.Skip;
                    if Model.FindFirst()then if Model."Amount Paid" < 0 then Error(AmountCannotBeNegativeErr, GenJnlLnBuffer."Account Type", GenJnlLnBuffer."Account No.");
                end;
            }
            trigger OnPreDataItem();
            begin
    ...
    requestpage
    {
        ...
        trigger OnOpenPage()begin
            ...
            if not Args.Get then Args.Insert;
    ...
    trigger OnPreReport()begin
        ...
        if CurrReport.Preview then Args."Test Print":=true;
        ;
        ReportsForNavPre;
    end;
    var AmountCannotBeNegativeErr: Label 'The total amount for %1 %2 cannot be negative', Comment='DO NOT TRANSLATE';
    procedure SetArgs(Value: Record "ForNAV Check Arguments")begin
        Args:=Value;
    end;
    procedure InputBankAccount()var BankAccount: Record "Bank Account";
    begin
    ...
    local procedure GetBankAccFromFirstGnlLine()var GenJournalLine: Record "Gen. Journal Line";
    begin
        ...
        if GenJournalLine.FindFirst()then Args."Bank Account No.":=GenJournalLine."Bal. Account No.";
    end;
    local procedure CheckSetupIsValid()var CheckSetup: Record "ForNAV Check Setup";
    begin
        CheckSetup.Get;
        if CheckSetup.Layout = CheckSetup.Layout::" " then CheckSetup.FieldError(CheckSetup.Layout);
    end;
    local procedure LoadWatermark()var CheckSetup: Record "ForNAV Check Setup";
    OutStream: OutStream;
    begin
        CheckSetup.Get;
        CheckSetup.CalcFields(CheckSetup.Watermark);
        if not CheckSetup.Watermark.Hasvalue then exit;
        ReportForNav.LoadWatermarkImage(CheckSetup.GetCheckWatermark);
    end;
    local procedure CreateGenJnlLnBuffer()var GenJnlLn: Record "Gen. Journal Line";
    begin
        ...
        begin
            ...
            if GenJnlLn.Find('-')then GenJnlLn.FieldError(GenJnlLn."Account Type");
            GenJnlLn.SetRange(GenJnlLn."Account Type");
            if GenJnlLn.FindSet then repeat GenJnlLnBuffer:=GenJnlLn;
                    GenJnlLnBuffer.Insert;
