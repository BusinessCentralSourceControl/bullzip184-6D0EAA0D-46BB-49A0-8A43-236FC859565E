Codeunit 6188690 "ForNAV Aging Calculation"
{
    procedure GetAging(Rec: Variant;
    var AgingBuffer: Record "ForNAV Aging Buffer";
    var Args: Record "ForNAV Aged Accounts Args.")var CustomerAging: Codeunit "ForNAV Customer Aging";
    VendorAging: Codeunit "ForNAV Vendor Aging";
    begin
        case GetAccountType(Rec)of AgingBuffer."account type"::Customer: CustomerAging.GetAging(Rec, AgingBuffer, Args);
        AgingBuffer."account type"::Vendor: VendorAging.GetAging(Rec, AgingBuffer, Args);
        end;
    end;
    procedure GetAgingWithCurrency(Rec: Variant;
    var AgingBuffer: Record "ForNAV Aging Buffer";
    var CurrAgingBuffer: Record "ForNAV Aging Buffer";
    var Args: Record "ForNAV Aged Accounts Args.";
    var TempCurrency: Record Currency temporary)var CustomerAging: Codeunit "ForNAV Customer Aging";
    VendorAging: Codeunit "ForNAV Vendor Aging";
    begin
        case GetAccountType(Rec)of AgingBuffer."account type"::Customer: CustomerAging.GetAgingWithCurrency(Rec, AgingBuffer, CurrAgingBuffer, Args, TempCurrency);
        AgingBuffer."account type"::Vendor: VendorAging.GetAgingWithCurrency(Rec, AgingBuffer, CurrAgingBuffer, Args, TempCurrency);
        end;
    end;
    local procedure GetAccountType(Rec: Variant): Integer var AgingBuffer: Record "ForNAV Aging Buffer";
    RecordRefLibrary: Codeunit "ForNAV RecordRef Library";
    UnsupportedTableErr: label 'Unsupported Table', Comment='DO NOT TRANSLATE';
    RecRef: RecordRef;
    begin
        RecordRefLibrary.ConvertToRecRef(Rec, RecRef);
        case RecRef.Number of Database::Customer: exit(AgingBuffer."account type"::Customer);
        Database::Vendor: exit(AgingBuffer."account type"::Vendor);
        else
            Error(UnsupportedTableErr);
        end;
    end;
    procedure UpdateCurrencyTotals(var AgingBuffer: Record "ForNAV Aging Buffer";
    var CurrAgingBuffer: Record "ForNAV Aging Buffer";
    ...
    procedure GetCurrencyCode(Value: Code[10]): Code[10]var GLSetup: Record "General Ledger Setup";
    begin
        if Value <> '' then exit(Value);
        GLSetup.Get;
        exit(GLSetup."LCY Code");
    end;
    procedure MoveValuesToPeriod(var AgingBuffer: Record "ForNAV Aging Buffer";
    PeriodIndex: Integer)var Fld: Record "Field";
    RecRef: RecordRef;
    FldRef: FieldRef;
    begin
