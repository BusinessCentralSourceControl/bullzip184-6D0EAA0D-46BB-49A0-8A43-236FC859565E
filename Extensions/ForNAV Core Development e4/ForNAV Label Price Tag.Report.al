...
{
    WordLayout = './Layouts/ForNAV Label Price Tag.docx';
    DefaultLayout = Word;
    ...
    dataset
    {
        dataitem(Label;"ForNAV Label")
        {
            ...
            column(ReportForNav_Label;ReportForNavWriteDataItem('Label', Label))
            {
            }
            trigger OnPreDataItem();
            begin
                ReportForNav.OnPreDataItem('Label', Label);
            end;
        }
    }
    requestpage
    ...
    trigger OnPreReport()var InStr: InStream;
    begin
        if TempBlob.FindFirst then begin
            TempBlob.CalcFields(Blob);
            TempBlob.Blob.CreateInstream(InStr);
            ReportForNav.SetAppendPdf('Label', InStr);
            TempBlob.Delete;
        end;
        ;
        ;
    ...
    var TempBlob: Record "ForNAV Core Setup" temporary;
    local procedure GetData()var LabelMgt: Codeunit "ForNAV Label Mgt.";
    begin
        LabelMgt.GetData(Label);
        LabelMgt.GetMergePDF(TempBlob);
        ReportForNavOpenDesigner:=Label."ForNAV Design";
    end;
    procedure SetMergePDF(var TempBlobIn: Record "ForNAV Core Setup")begin
    ...
    begin
        ...
        case(dataItemId)of 'Label': begin
            currLanguage:=GlobalLanguage;
            GlobalLanguage:=1033;
            jsonObject.Add('DataItem$Label$CurrentKey$Text', Label.CurrentKey);
