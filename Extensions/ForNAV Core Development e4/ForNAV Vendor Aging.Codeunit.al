Codeunit 6188692 "ForNAV Vendor Aging"
{
    ...
    var TempVendorLedgerEntry: Record "Vendor Ledger Entry" temporary;
    procedure GetAging(var Vend: Record Vendor;
    ...
    procedure GetAgingWithCurrency(var Vend: Record Vendor;
    var AgingBuffer: Record "ForNAV Aging Buffer";
    ...
    local procedure GetBasedOnDetailedEntry(var Vend: Record Vendor;
    var Args: Record "ForNAV Aged Accounts Args.")var DetailedVendorLedgEntry: Record "Detailed Vendor Ledg. Entry";
    ...
    begin
        ...
        if DetailedVendorLedgEntry.FindSet then repeat if VendorLedgerEntry.Get(DetailedVendorLedgEntry."Vendor Ledger Entry No.")then if VendorLedgerEntry.Open then begin
                        VendorLedgerEntry.SetRange("Date Filter", 0D, Args."Ending Date");
                        VendorLedgerEntry.CalcFields("Remaining Amount");
                        if VendorLedgerEntry."Remaining Amount" <> 0 then InsertTemp(VendorLedgerEntry);
                    end;
            until DetailedVendorLedgEntry.Next = 0;
    end;
    local procedure GetBasedOnOpenEntry(var Vend: Record Vendor;
    var Args: Record "ForNAV Aged Accounts Args.")var VendorLedgerEntry: Record "Vendor Ledger Entry";
    begin
        ...
        if VendorLedgerEntry.FindSet then repeat if Args."Aging By" = Args."aging by"::"Posting Date" then begin
                    VendorLedgerEntry.CalcFields(VendorLedgerEntry."Remaining Amt. (LCY)");
                    if VendorLedgerEntry."Remaining Amt. (LCY)" <> 0 then InsertTemp(VendorLedgerEntry);
                end
                else
                    InsertTemp(VendorLedgerEntry);
            until VendorLedgerEntry.Next = 0;
    end;
    local procedure CreateAgingBuffer(var Vend: Record Vendor;
    var AgingBuffer: Record "ForNAV Aging Buffer";
    var CurrAgingBuffer: Record "ForNAV Aging Buffer";
    var Args: Record "ForNAV Aged Accounts Args.";
    var TempCurrency: Record Currency temporary)var DetailedVendorLedgEntry: Record "Detailed Vendor Ledg. Entry";
    AgingCalculation: Codeunit "ForNAV Aging Calculation";
    PeriodIndex: Integer;
    begin
        if TempVendorLedgerEntry.FindSet then repeat AgingBuffer.Init;
                ...
                if not Args."Print Amounts in LCY" then AgingBuffer."Currency Code":=AgingCalculation.GetCurrencyCode(TempVendorLedgerEntry."Currency Code");
                ...
                AgingBuffer."Document Type":=TempVendorLedgerEntry."Document Type";
                ...
                if DetailedVendorLedgEntry.FindSet then repeat if(DetailedVendorLedgEntry."Entry Type" = DetailedVendorLedgEntry."entry type"::"Initial Entry") and (TempVendorLedgerEntry."Posting Date" > Args."Ending Date") and (Args."Aging By" <> Args."aging by"::"Posting Date")then begin
                            if TempVendorLedgerEntry."Document Date" <= Args."Ending Date" then DetailedVendorLedgEntry."Posting Date":=TempVendorLedgerEntry."Document Date"
                            else if(TempVendorLedgerEntry."Due Date" <= Args."Ending Date") and (Args."Aging By" = Args."aging by"::"Due Date")then DetailedVendorLedgEntry."Posting Date":=TempVendorLedgerEntry."Due Date";
                        end;
                        if(DetailedVendorLedgEntry."Posting Date" <= Args."Ending Date") or (TempVendorLedgerEntry.Open and (Args."Aging By" = Args."aging by"::"Due Date") and (TempVendorLedgerEntry."Due Date" > Args."Ending Date") and (TempVendorLedgerEntry."Posting Date" <= Args."Ending Date"))then begin
                            if DetailedVendorLedgEntry."Entry Type" in[DetailedVendorLedgEntry."entry type"::"Initial Entry", DetailedVendorLedgEntry."entry type"::"Unrealized Loss", DetailedVendorLedgEntry."entry type"::"Unrealized Gain", DetailedVendorLedgEntry."entry type"::"Realized Loss", DetailedVendorLedgEntry."entry type"::"Realized Gain", DetailedVendorLedgEntry."entry type"::"Payment Discount", DetailedVendorLedgEntry."entry type"::"Payment Discount (VAT Excl.)", DetailedVendorLedgEntry."entry type"::"Payment Discount (VAT Adjustment)", DetailedVendorLedgEntry."entry type"::"Payment Tolerance", DetailedVendorLedgEntry."entry type"::"Payment Discount Tolerance", DetailedVendorLedgEntry."entry type"::"Payment Tolerance (VAT Excl.)", DetailedVendorLedgEntry."entry type"::"Payment Tolerance (VAT Adjustment)", DetailedVendorLedgEntry."entry type"::"Payment Discount Tolerance (VAT Excl.)", DetailedVendorLedgEntry."entry type"::"Payment Discount Tolerance (VAT Adjustment)"]then begin
                                if not Args."Print Amounts in LCY" then AgingBuffer.Amount+=DetailedVendorLedgEntry.Amount
                                else
                            ...
                            if DetailedVendorLedgEntry."Posting Date" <= Args."Ending Date" then begin
                                if not Args."Print Amounts in LCY" then AgingBuffer.Balance+=DetailedVendorLedgEntry.Amount
                                else
    ...
    local procedure InsertTemp(var VendorLedgerEntry: Record "Vendor Ledger Entry")begin
        if TempVendorLedgerEntry.Get(VendorLedgerEntry."Entry No.")then exit;
